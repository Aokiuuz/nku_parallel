...
static ALWAYS_INLINE uint32x4_t ByteSwap32Simd(uint32x4_t value)
{
	return vorrq_u32(
...
static ALWAYS_INLINE void InitMd5StateSimd(uint32x4_t state[4])
{
...
static ALWAYS_INLINE void Md5TransformSimd(const uint32x4_t x[16], uint32x4_t state[4])
{
...
static bool BuildSingleBlockWordsSimd(const string input[4], uint32x4_t x[16])
{
	...
	for (int lane = 0; lane < 4; lane += 1)
	{
		...
		const unsigned char *data = reinterpret_cast<const unsigned char *>(input[lane].data());
		for (size_t i = 0; i < length; i += 1)
		{
			lanes[i >> 2][lane] |= static_cast<uint32_t>(data[i]) << ((i & 3) * 8);
		}
...
static void PackSerialHashes(const string input[4], uint32x4_t state[4])
{
	bit32 scalar[4][4];
	uint32_t lanes[4];
	for (int lane = 0; lane < 4; lane += 1)
	{
		MD5Hash(input[lane], scalar[lane]);
	}
	for (int word = 0; word < 4; word += 1)
	{
		for (int lane = 0; lane < 4; lane += 1)
		{
			lanes[lane] = scalar[lane][word];
		}
		state[word] = vld1q_u32(lanes);
	}
}

static void MD5Hash_simdversion_slow(const string input[4], uint32x4_t state[4])
{
	Byte *paddedMessage[4];
	int messageLength[4];
	for (int lane = 0; lane < 4; lane += 1)
	{
		paddedMessage[lane] = StringProcess(input[lane], &messageLength[lane]);
	}

	for (int lane = 1; lane < 4; lane += 1)
	{
		if (messageLength[lane] != messageLength[0])
		{
			for (int i = 0; i < 4; i += 1)
			{
				delete[] paddedMessage[i];
			}
			PackSerialHashes(input, state);
			return;
		}
	}

	const int n_blocks = messageLength[0] / 64;
	InitMd5StateSimd(state);
	for (int block = 0; block < n_blocks; block += 1)
	{
		...
		for (int word = 0; word < 16; word += 1)
		{
			for (int lane = 0; lane < 4; lane += 1)
			{
				lanes[lane] = DecodeWord(paddedMessage[lane], 4 * word + block * 64);
			}
			x[word] = vld1q_u32(lanes);
		}
		Md5TransformSimd(x, state);
	}

	for (int word = 0; word < 4; word += 1)
	{
		state[word] = ByteSwap32Simd(state[word]);
	}
	for (int lane = 0; lane < 4; lane += 1)
	{
		delete[] paddedMessage[lane];
	}
}
...
{
	uint32x4_t x[16];
	if (!BuildSingleBlockWordsSimd(input, x))
	{
		MD5Hash_simdversion_slow(input, state);
		return;
	}
